...
const COUNT_CLS = { red: styles.countRed, yellow: styles.countYellow, blue: styles.countBlue, green: styles.countGreen, gray: styles.countGray }
  ...
  return (
    <div className={`${styles.section} ${open ? styles.open : ''}`}>
      <div className={styles.head} onClick={() => setOpen(o => !o)}>
        ...
        <span className={`${styles.count} ${COUNT_CLS[color]}`}>{issues.length}</span>
            ...
            : issues.map((issue, idx) => (
              <div key={idx} className={styles.item}
                ...
                <div className={styles.meta}>
                  ...
                  <span className={`${styles.sev} ${sevCls(issue.severity)}`}>{issue.severity ?? 'info'}</span>
                </div>
              </div>
            ))
          }
